      ...
      on roles(role) do
        ...
        workers.each_pair do |queue, number_of_workers|
          ...
          number_of_workers.times do
            ...
            run "cd #{current_path} ; nohup #{SSHKit.config.command_map[:rake]} RACK_ENV=#{rails_env} RAILS_ENV=#{rails_env} QUEUE='#{queue}' PIDFILE=#{pid} BACKGROUND=yes #{'VERBOSE=1 ' if fetch(:resque_verbose)}INTERVAL=#{fetch(:interval)} #{'environment ' if fetch(:resque_environment_task)}resque:work #{output_redirection}", :pty => false
            worker_id += 1
          end
        end
      end
